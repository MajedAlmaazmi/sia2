...
const NewRequestPage: React.FC = ({ imams, freelancers }) => {
  const {
    register,
    handleSubmit,
    watch,
  ...
  // console.log(selectedImam);
  const onSubmit = (data: RequestForm) => {
    console.log(data);
    fetch("https://sunset-nosy-toast.glitch.me/requests", {
      method: "POST",
      headers: {
  ...
  return (
    <div className="border-2 flex flex-col p-4 rounded  border-black mt-4 mb-6 ml-4 mr-2 grow">
      <h1 className="text-4xl mb-4 font-bold">إنشاء إجازة جديدة</h1>
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        {status !== "PENDING" && (
          <div>
            <input type="checkbox" {...register("send", {})} />
            <label>إرسال رسالة نصية عبر الواتساب</label>
          </div>
        )}
...
export const getServerSideProps: GetServerSideProps<Props> = async () => {
  ...
  return { props: { imams, freelancers } };
};
